...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const tempsBlocage = parseInt(localStorage.getItem('maodo_temps_blocage') || '0')
      const restantMs = tempsBlocage + DUREE_BLOCAGE - Date.now()
      if (restantMs <= 0) {
        localStorage.removeItem('maodo_bloque')
        localStorage.removeItem('maodo_tentatives')
        localStorage.removeItem('maodo_temps_blocage')
        setEstBloque(false)
        setError('')
        setTentatives(0)
  ...
  const verifierBlocageInitial = () => {
    const bloque = localStorage.getItem('maodo_bloque')
    const tempsBlocage = parseInt(localStorage.getItem('maodo_temps_blocage') || '0')
    const t = parseInt(localStorage.getItem('maodo_tentatives') || '0')
    setTentatives(t)
  ...
  const handleLogin = async () => {
    ...
    if (authError) {
      ...
      localStorage.setItem('maodo_tentatives', nouvellesTentatives.toString())

      if (nouvellesTentatives >= MAX_TENTATIVES) {
        localStorage.setItem('maodo_bloque', 'true')
        localStorage.setItem('maodo_temps_blocage', Date.now().toString())
        setEstBloque(true)
